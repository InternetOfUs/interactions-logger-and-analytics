...
class ProjectToAppIdMigration(MigrationAction):
    ...
    def apply(self, es: Elasticsearch) -> None:
        ...
        for analytic in analytics:
            analytic['_source']['descriptor']['project'] = "cG37pczAJx" if datetime.fromisoformat(analytic['_source']['timestamp']) < datetime(2021, 3, 12) else "2kUw54aeVP"
            index = analytic['_index']
            es.delete(index=index, id=analytic['_id'], doc_type=analytic['_type'])
            index = index.replace("wenet-ask-for-help-aalborg", "cg37pczajx" if datetime.fromisoformat(analytic['_source']['timestamp']) < datetime(2021, 3, 12) else "2kuw54aevp")
