...
if __name__ == '__main__':
    ...
    if extension == ".csv":
        analytics_file = csv.writer(open(args.afile, "w"))
    elif extension == ".tsv":
        analytics_file = csv.writer(open(args.afile, "w"), delimiter="\t")
    else:
    ...
    analytics_file.writerow(["app id", args.appid])
    analytics_file.writerow(["project", args.project])
    analytics_file.writerow(["from", created_from])
    analytics_file.writerow(["to", created_to])
    analytics_file.writerow([])
    analytics_file.writerow(["metric", "count", "description"])

    total_users = UserMetric("u:total")
    total_users_result = operations.get_analytic(time_range, total_users)
    analytics_file.writerow(["total users", total_users_result["count"], "The total number of users of the application"])

    active_users = UserMetric("u:active")
    active_users_result = operations.get_analytic(time_range, active_users)
    analytics_file.writerow(["active users", active_users_result["count"], "The number of users who used the application"])

    engaged_users = UserMetric("u:engaged")
    engaged_users_result = operations.get_analytic(time_range, engaged_users)
    analytics_file.writerow(["engaged users", engaged_users_result["count"], "The number of users who received a notification from the platform (incentive, prompt, badge, ..)"])

    new_users = UserMetric("u:new")
    new_users_result = operations.get_analytic(time_range, new_users)
    analytics_file.writerow(["new users", new_users_result["count"], "The number of new users who activated the application during the period of this analysis"])
    ...
    analytics_file.writerow(["total messages", total_messages, "The total number of messages exchanged"])
    ...
    analytics_file.writerow(["messages from users", request_messages, "The number of messages sent by users (textual messages, clicked buttons and commands)"])
    ...
    analytics_file.writerow(["user textual messages", text_requests, "The number of textual messages sent by the users"])
    ...
    analytics_file.writerow(["user action messages", action_requests, "The number of action messages (buttons and commands) sent by the users"])
    ...
    analytics_file.writerow(["messages from bot", response_messages, "The number of messages sent by the application"])
    ...
    analytics_file.writerow(["messages from wenet", notification_messages, "The number of messages sent by the WeNet platform"])

    tasks = task_manager_connector.get_tasks(args.appid, created_from, created_to)
    analytics_file.writerow(["questions ", len(tasks), "The number of questions asked by the users"])
    ...
    analytics_file.writerow(["question reports", report_question_transactions, "The number of reports created for questions (e.g. usually for malicious contents)"])

    not_answer_transactions = transaction_labels.count(LABEL_NOT_ANSWER_TRANSACTION)
    analytics_file.writerow(["can't help", not_answer_transactions, "The number of times a user cannot answer a question (by clicking on the button “Can't help”)"])

    answer_transactions = transaction_labels.count(LABEL_ANSWER_TRANSACTION)
    analytics_file.writerow(["answer", answer_transactions, "The number of answers provided by the users"])

    report_answer_transactions = transaction_labels.count(LABEL_REPORT_ANSWER_TRANSACTION)
    analytics_file.writerow(["answer reports", report_answer_transactions, "The number of reports created for answers (e.g. usually for malicious contents)"])

    best_answer_transactions = transaction_labels.count(LABEL_BEST_ANSWER_TRANSACTION)
    analytics_file.writerow(["approved answers", best_answer_transactions, "The number answers accepted as the correct/useful ones"])

    more_answer_transactions = transaction_labels.count(LABEL_MORE_ANSWER_TRANSACTION)
    analytics_file.writerow(["ask more people", more_answer_transactions, "The number of times a user asked WeNet for more answers (by clicking on the button “Ask more people”)"])

    # all questions and their chosen answers
    name, extension = os.path.splitext(args.qfile)
    if extension == ".csv":
        questions_file = csv.writer(open(args.qfile, "w"))
    elif extension == ".tsv":
        questions_file = csv.writer(open(args.qfile, "w"), delimiter="\t")
    else:
    ...
    questions_file.writerow(["app id", args.appid])
    questions_file.writerow(["project", args.project])
    questions_file.writerow(["from", created_from])
    questions_file.writerow(["to", created_to])
    questions_file.writerow([])
    ...
    if extension == ".csv":
        users_file = csv.writer(open(args.ufile, "w"))
    elif extension == ".tsv":
        users_file = csv.writer(open(args.ufile, "w"), delimiter="\t")
    else:
    ...
    for user_id in user_ids:
        ...
        if user_cohort:
            users_file.writerow([user.profile_id, user.name.first, user.name.last, user.email, user.gender.name, user_cohort])
        else:
            users_file.writerow([user.profile_id, user.name.first, user.name.last, user.email, user.gender.name])
